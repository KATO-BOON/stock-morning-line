...
def _classify_trend(price: float, ma25: float, ma75: float,
    ...
    """
    # --- 重み付けは scripts/backtest.py の検証結果（500日・8,400判定）に基づく ---
    score = 0.0
    ...
    # モメンタム（検証で最も超過リターンとの相関が高かった: +0.084）
    score += mom_20d * 0.15   # 20日の地合い
    score += mom_5d * 0.10    # 直近の勢い
    # 注: 市場対比(rel_str)はスコアに加算しない。
    #     rel_str = mom_20d − 日経20日騰落率 で、日経分は同日の全銘柄で共通のため
    #     銘柄間の順位付けにはmom_20dと同じ情報しか持たない（＝モメンタムの二重計上になる）。
    #     表示用としては有用なので値自体は保持する。
    # 出来高比: 検証での相関は +0.005 とほぼ無関係だったため影響を小さくした
    if vol_ratio >= 1.5:
        score += 0.3
    elif vol_ratio < 0.7:
        score -= 0.2
    # RSI: 検証では「高RSIほど超過リターンが良い」(相関+0.067)。
    #      当初の過熱ペナルティは逆効果だったため撤回し、
    #      極端値(85以上)のみリスク管理として軽く減点する。
    if rsi14 >= 85:
        score -= 1.0
    # 5日で+15%超の急騰は反動リスクとして軽く減点
    if mom_5d > 15:
        score -= 0.5
